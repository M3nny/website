...
const remarkUraraFm =
  ...
  (
    tree: Node<Data>,
    {
      data,
      filename,
    }: { data: { fm?: Record<string, unknown> }; filename?: string }
  ) => {
    ...
    data.fm.path = join(
      dir,
      `/${name}`.replace("/+page", "").replace(".svelte", "")
    );
    // Generate ToC
    if (data.fm.toc !== false) {
      const [slugs, toc]: [
        slugs: Slugger,
        toc: { depth: number; title: string; slug: string }[]
      ] = [new Slugger(), []];
...
const remarkUraraSpoiler = () => (tree: Node<Data>) =>
  visit(tree, "paragraph", (node: any) => {
    ...
    if (re.test(children[0].value)) {
      ...
      children[0].value = text.replace(
        re,
        (_match: unknown, p1: string) => `<span class="spoiler">${p1}</span>`
      );
    }
    return node;
  });
  ...
  highlight: {
    highlighter: async (code, lang, meta) => {
      ...
      if (fence?.twoslash === true)
        twoslash = runTwoSlash(code, lang as string);
