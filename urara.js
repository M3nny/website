...
const log = (color, msg, dest) => console.log(chalk.dim(new Date().toLocaleTimeString() + " ") +
...
const cpFile = (src, { stat = "copy", dest = path.join(check(path.parse(src).ext.slice(1)), src.slice(6)), } = {}) => config.extensions.images.includes(path.parse(src).ext.slice(1))
    ? fs
...
const mkDir = (src, { dest = [
    path.join("src/routes", src.slice(6)),
    path.join("static", src.slice(6)),
    path.join("src/static", src.slice(6)),
], } = {}) => {
