<script lang="ts">
  ...
  const stringPublished = new Date(
    post.published ?? post.created
  ).toLocaleString(date.locales, date.options);
  const stringUpdated = new Date(
    post.updated ?? post.published ?? post.created
  ).toLocaleString(date.locales, date.options);
  const jsonPublished = new Date(post.published ?? post.created).toJSON();
  const jsonUpdated = new Date(
    post.updated ?? post.published ?? post.created
  ).toJSON();
</script>

<div
  class:md:mb-4={!preview && post.type !== "article"}
  class="flex font-semibold gap-1.5"
>
  <a
