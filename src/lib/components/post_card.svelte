<script lang="ts">
  ...
  if (browser && !preview)
    storedPosts.subscribe((storedPosts: Urara.Post[]) => {
      index = storedPosts.findIndex(
        (storedPost) => storedPost.path === post.path
      );
      prev = storedPosts
        .slice(0, index)
        .reverse()
...
>
  ...
  <div
    class={`card-body gap-0 ${
      preview && post.type === "article" && post.image
    ...
    <div class="flex flex-col gap-2">
      {#if post.image && !preview}
        <figure
          class={`md:order-last rounded-box shadow-xl mb-4 ${
            post.type === "article"
              ? "flex-col gap-2 -mx-4 -mt-8 md:mt-0"
              : "flex-col -mx-8"
          }`}
        >
          <Image
      ...
      {#if post.title}
        {#if preview}
          ...
          >
            <a itemprop="url" class="u-url p-name" href={post.path}
              >{post.title ?? post.path.slice(1)}</a
            >
          </h2>
        {:else}
          <h1 itemprop="name headline" class="card-title text-3xl mb-8 p-name">
      ...
      {#if post.summary}
        <p
          ...
          class:hidden={!preview || post.type !== "article"}
          class="p-summary mb-auto"
        >
          {post.summary}
        </p>
      {/if}
    ...
    {#if !preview && post.tags}
      ...
      <div>
        {#each post.tags as tag}
          <a
            href="/?tags={tag}"
            class="btn btn-sm btn-ghost normal-case mt-2 mr-2 p-category"
          >
            #{tag}
          </a>
        {/each}
