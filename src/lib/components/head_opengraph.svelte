...
<svelte:head>
  ...
  {#if post}
    ...
    <meta
      property="og:title"
      content={post.title ?? post.summary ?? post.path.slice(1)}
    />
    {#if post.summary}
    ...
    {#if post.image}
      <meta
        ...
        content={(post.image.startsWith("http")
          ? ""
          : site.protocol + site.domain) + post.image}
      />
      <meta name="twitter:card" content="summary_large_image" />
    {:else}
      <meta
        property="og:image"
        content={maskable["512"].src ?? any["512"].src ?? any["192"].src}
      />
    ...
    <meta
      ...
      content={post.published ?? post.created}
    />
    <meta
      property="article:modified_time"
      content={post.updated ?? post.published ?? post.created}
    />
  {:else}
    ...
    {#if page}
      ...
      <meta
        property="og:url"
        content={site.protocol + site.domain + page.path}
      />
    {:else}
