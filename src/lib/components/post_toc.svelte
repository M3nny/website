          ...
          headings.forEach((heading) =>
            heading.isIntersecting
              ...
              : (intersecting = intersecting.filter(
                  (h) => h !== heading.target.id
                ))
          ),
        { rootMargin: "-64px 0px 0px 0px" }
      );
      const articleObserver = new IntersectionObserver(
        (article) => (intersectingArticle = article[0].isIntersecting)
      );
      Array.from(
        document.querySelectorAll("main h2, main h3, main h4, main h5, main h6")
      ).forEach((element) => headingsObserver.observe(element));
  ...
  $: if (bordered)
    toc.forEach((heading) =>
      bordered.includes(heading.slug!)
        ? document
            .getElementById(`toc-link-${heading.slug}`)
